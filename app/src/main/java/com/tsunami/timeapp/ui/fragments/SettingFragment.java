package com.tsunami.timeapp.ui.fragments;
...
import com.tsunami.timeapp.adpater.ColorsListAdapter;
import com.tsunami.timeapp.injector.component.DaggerFragmentComponent;
import com.tsunami.timeapp.injector.module.FragmentModule;
import com.tsunami.timeapp.mvp.presenters.impl.SettingPresenter;
import com.tsunami.timeapp.mvp.views.impl.SettingView;
...
import com.tsunami.timeapp.utils.ThemeUtils;
...
public class SettingFragment extends PreferenceFragment implements SettingView{
    ...
    // 2 绑定Activity 在Fragment绑定中，对Fragment中的Activity成员变量进行初始化，
    // 防止每次调用getActivity造成性能损失。
    @Override
    ...
    // 2 与Activity中的findViewById()类似，Preference也类似，
    // 需要查找到每个控件，只不过此处通过 key 属性来进行查找。
    @Override
    ...
    // 2 ListView自定义 之后，通过view.findViewById(android.R.id.list)
    // 获取ListView，可以对该ListView进行自定义。view为重写SettingFragment下的onViewCreated(View view,
    // Bundle savedInstanceState)方法中的参数。
    @Override
