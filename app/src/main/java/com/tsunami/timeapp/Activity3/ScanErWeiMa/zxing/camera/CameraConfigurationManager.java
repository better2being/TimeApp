...
final class CameraConfigurationManager {
	...
	void setDesiredCameraParameters(Camera camera, boolean safeMode) {
		...
		SharedPreferences prefs = ScanActivity.preferences(context);
		initializeTorch(parameters, prefs, safeMode);
		String focusMode = null;
		if (prefs.getBoolean(ScanActivity.KEY_AUTO_FOCUS, true)) {
			if (safeMode || prefs.getBoolean(ScanActivity.KEY_DISABLE_CONTINUOUS_FOCUS, false)) {
	...
	void setTorch(Camera camera, boolean newSetting) {
		...
		camera.setParameters(parameters);
		SharedPreferences prefs = ScanActivity.preferences(context);
		boolean currentSetting = prefs.getBoolean(ScanActivity.KEY_FRONT_LIGHT, false);
		if (currentSetting != newSetting) {
			SharedPreferences.Editor editor = prefs.edit();
			editor.putBoolean(ScanActivity.KEY_FRONT_LIGHT, newSetting);
			editor.commit();
		}
	}
